    ...
    if ((geoSettings != null && geoSettings.geocodeContacts__c) || GW_Geocoder.isTest) {        
        ...
        if (!conids.isEmpty()) {
        	try {
        		...
        		// if exception is thrown it's usually because we were calling this from another future method (dupeblocker etc.)
        		// if we change this to use batch processing, we should remove this try/catch
